import requests
import pyodbc
from datetime import datetime
import sys

# --- CONFIGURAÇÕES ---
data_hoje = datetime.now().strftime('%m-%d-%Y')

# URL corrigida: Removemos o $top=1 para ele trazer todos os boletins do dia
url_api = f"https://olinda.bcb.gov.br/olinda/servico/PTAX/versao/v1/odata/CotacaoMoedaDia(moeda=@moeda,dataCotacao=@dataCotacao)?@moeda='EUR'&@dataCotacao='{data_hoje}'&$format=json"

dados_conexao = (
    "Driver={ODBC Driver 17 for SQL Server};"
    "Server=CORPORATIVOMTCS;"
    "Database=Financeiro;"
    "Trusted_Connection=yes;"
)


def importar_euro_diario():
    try:
        response = requests.get(url_api)
        dados = response.json()

        if not dados['value']:
            print(
                f"Nenhuma cotação de Euro disponível para hoje ({data_hoje}).")
            return

        # A MÁGICA AQUI: O índice [-1] garante que vamos pegar o ÚLTIMO boletim do dia (Fechamento/PTAX)
        item = dados['value'][-1]

        valor_compra = item['cotacaoCompra']
        valor_venda = item['cotacaoVenda']
        data_api = item['dataHoraCotacao'].split(' ')[0]

        conn = pyodbc.connect(dados_conexao)
        cursor = conn.cursor()

        query = """
        IF NOT EXISTS (SELECT 1 FROM CotacaoEuro WHERE DataCotacao = ?)
        BEGIN
            INSERT INTO CotacaoEuro (DataCotacao, ValorCompra, ValorVenda)
            VALUES (?, ?, ?)
        END
        """

        cursor.execute(query, data_api, data_api, valor_compra, valor_venda)
        conn.commit()
        print(
            f"Sucesso: PTAX Final do Euro de {data_api} inserida/verificada. Compra: {valor_compra}")

    except Exception as e:
        print(f"Erro: {e}")
    finally:
        if 'conn' in locals():
            conn.close()


if __name__ == "__main__":
    importar_euro_diario()
